...
/**
Added by Hubbard for an intermediate return point
*/
var xyzFormat = createFormat({decimals: 4, forceDecimal: true, trim: true});
var retractX = 15.7
var retractZ = 8.0

description = "FANUC Hubbard";
// === Custom G74 Configuration ===
var useG74forPeckDrilling = true; // Set to false to use G81 instead
...
function onOpen() {
  ...
  writeBlock(conditional(getProperty("type") != "A", gAbsIncModal.format(90)), formatFeedMode(FEED_PER_REVOLUTION), gPlaneModal.format(18));
...
function onCycle(cycle) {
  var z = cycle.bottom.z;
  var f = cycle.feedrate;
  var k = cycle.incrementalDepth;

  switch (cycle.type) {
    case "drilling-peck":
      if (useG74forPeckDrilling) {
        writeBlock("G74",
          "Z" + xyzFormat.format(z),
          "K" + xyzFormat.format(k),
          "F" + feedFormat.format(f)
        );
      } else {
        writeBlock("G81",
          "Z" + xyzFormat.format(z),
          "R" + xyzFormat.format(cycle.retract.z),
          "F" + feedFormat.format(f)
        );
      }
      break;
    ...
    default:
      error(localize("Unsupported cycle type: " + cycle.type));
      break;
  }
}
...
function writeRetract() {
  ...
  for (var i = 0; i < words.length; ++i) {
    ...
    case "G28":
      gAbsIncModal.reset();
      writeBlock(gFormat.format(0), "X" + xyzFormat.format(retractX), "Z" + xyzFormat.format(retractZ));
